        ...
        {sliderData.map((slide) => (
          ...
          >
            ...
            <div className="flex items-center flex-1 justify-center">
              <Image className="md:w-72 w-48" src={slide.imgSrc} alt="product" />
            </div>
          </div>
        ))}
      </div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-[90%] max-w-md relative">
            ...
            <p className="text-sm text-gray-600">{selectedProduct.offer}</p>
            <Image
              src={selectedProduct.imgSrc}
              alt="Product"
              className="my-4 w-full h-auto object-contain"
            />
            <button className="mt-2 bg-orange-600 text-white px-4 py-2 rounded-full w-full font-medium">
              Proceed to Checkout
            </button>
          </div>
        </div>
